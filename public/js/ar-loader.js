...
function getAudioContext() {
    if (!audioContext) {
        ...
        console.log(
            "[ar-loader] AudioContext + GainNode created, " +
                ...
                try {
                    ctx.decodeAudioData(
                        ...
                        function (buffer) {
                            ...
                            console.log(
                                "[ar-loader] AudioBuffer decoded + cached:",
                                audioSrc,
                            );
                            resolve(buffer);
                        },
    ...
    decodeAudioToBuffer(audioSrc)
        .then(async function (buffer) {
            ...
            if (ctx.state === "suspended") {
                console.log(
                    "[ar-loader] AudioContext suspended — waiting for resume...",
                );
                ctx.resume().then(function () {
                    console.log(
                        "[ar-loader] AudioContext resumed — playing audio",
                    );
                    doPlay();
                });
            } else {
                console.log(
                    "[ar-loader] AudioContext running — playing audio now",
                );
                doPlay();
            }
        })
        .catch(function (e) {
            console.error(
                "[ar-loader] Web Audio failed, fallback to HTML Audio:",
                audioSrc,
                e,
            );
        ...
        function startAnimationLoop() {
            ...
            (function animationLoop() {
                ...
                if (globalClock) {
                    ...
                    if (dt > 0.1) dt = 0.1;
                    _arAnimationMixers.forEach(function (m) {
                        m.update(dt);
                    });
                }
            })();
